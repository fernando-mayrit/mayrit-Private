...
def _comisiones_premium_por_mes(db: Session, prog: Programa) -> dict[str, Decimal]:
    """Σ comisión del coverholder del Premium, por mes (YYYY-MM), de los binders del programa."""
    ...
    if not bids:
        return out
    for pbdx, com in db.execute(
        select(BdxLinea.premium_bdx, func.sum(BdxLinea.commission_coverholder_amount))
        ...
        out[pbdx.strftime("%Y-%m")] += (com or D0)
    return out
...
class MesComision(BaseModel):
    periodo: str
    comision_premium: Decimal           # estimación (del Premium)
...
def _mes_de_liq(db: Session, liq: ComisionLiquidacion, est_premium: Decimal) -> MesComision:
    r = db.get(Recibo, liq.recibo_id) if liq.recibo_id else None
    com = _comision_efectiva(liq)
    return MesComision(
        periodo=liq.periodo, comision_premium=_q2(est_premium), liq_id=liq.id, estado=liq.estado,
...
def listar_iberian(db: Session = Depends(get_db)):
    ...
    est = _comisiones_premium_por_mes(db, prog)
    ...
    for per in sorted(set(est) | set(liqs), reverse=True):
        l = liqs.get(per)
        if l:
            out.append(_mes_de_liq(db, l, est.get(per, D0)))
        else:
            out.append(MesComision(periodo=per, comision_premium=_q2(est.get(per, D0))))
    return out
...
def preparar_iberian(periodo: str, db: Session = Depends(get_db)):
    ...
    est = _comisiones_premium_por_mes(db, prog).get(periodo, D0)
    fecha = _dia1(periodo)
    liq = ComisionLiquidacion(
        fuente="Iberian", programa_id=prog.id, periodo=periodo, fecha=fecha,
        comision_premium=_q2(est), cedida_pct=Decimal(85), retenida_pct=Decimal(15),
...
def ratificar(liq_id: int, payload: RatificarIn, db: Session = Depends(get_db)):
    ...
    db.commit()
    prog = _programa_iberian(db)
    est = _comisiones_premium_por_mes(db, prog).get(liq.periodo, D0) if prog else D0
    return _mes_de_liq(db, liq, est)
